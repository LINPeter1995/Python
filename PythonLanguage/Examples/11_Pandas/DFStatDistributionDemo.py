...
def histogram(df):
    ...
    # KDE(Kernel Density Estimation) 比起直方圖的離散條狀分佈提供更平滑的分佈趨勢
...
def main():
    # 設定隨機種子以確保結果一致
    np.random.seed(10)
    # 生成 30 位學生的分數 (模擬一個常見的分佈)
    # 平均75分，標準差10
    exam_scores = np.random.normal(loc=75, scale=10, size=30)
    # 確保分數介於[0,100]：分數<0為0；分數>100為100
    exam_scores = np.clip(exam_scores, 0, 100)
    # 建立 DataFrame
    df = DataFrame({'Score': exam_scores})
    print("測試資料")
    print(f"df:\n{df}")
